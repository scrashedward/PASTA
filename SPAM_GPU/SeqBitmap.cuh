...
const unsigned int Bit32Table[32] = {2147483648UL,
                                     1073741824UL,
                                     536870912UL,
                                     268435456UL,
                                     134217728,
...
public:
  ...
  void SBitmapDelete() { delete sBitmapList[0]; }
  ...
  void CudaMemcpy(bool deviceToHost = false) {
    cudaError_t error;
    if (!deviceToHost) {
      if ((error = cudaMemcpy(gpuMemList[0], bitmapList[0],
                              sizeof(int) * sizeSum, cudaMemcpyHostToDevice)) !=
          cudaSuccess) {
      ...
      if ((error = cudaMemcpy(bitmapList[0], gpuMemList[0],
                              sizeof(int) * sizeSum, cudaMemcpyDeviceToHost)) !=
          cudaSuccess) {
        cout << "cudaError: " << error << endl;
  ...
  void CudaFree() { gpuMemPool.push(gpuMemList[0]); }
  ...
  void SBitmapCudaFree() { gpuMemPool.push(gpuSMemList[0]); }

  void SBitmapCudaMemcpy() {
    cudaError_t error =
        cudaMemcpy(sBitmapList[0], gpuSMemList[0], sizeof(int) * sizeSum,
