...
public:
	...
	void SBitmapConversion(cudaStream_t cudaStream)
	{
		if (curSize == 0) return;
		gpuErrchk(cudaMemcpyAsync(gsList, sourceList, sizeof(int*) * curSize, cudaMemcpyHostToDevice, cudaStream));
		gpuErrchk(cudaMemcpyAsync(gdList, sBitmapList, sizeof(int*) * curSize, cudaMemcpyHostToDevice, cudaStream));
		int pad = 0;
		for (int i = 0; i < 4; ++i)
		{
